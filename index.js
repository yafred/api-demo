...
function parseFragment(fragmentNode, parentNode) {
  ...
  (_a = fragment.parent) !== null && _a !== void 0
    ? _a
    : (fragment.parent = parentNode !== null && parentNode !== void 0 ? parentNode : null);
...
class t extends r {
  constructor(r) {
    (super(), (this.value = void 0), (this.isOk = true), (this.isErr = false), (this.value = r));
  }
  _chain(r, t) {
    return r(this.value);
  }
...
class e extends r {
  constructor(r) {
    (super(), (this.error = void 0), (this.isOk = false), (this.isErr = true), (this.error = r));
  }
  _chain(r, t) {
    return t(this.error);
  }
...
class Position {
  ...
  outcome(ctx) {
    ...
    ctx = ctx || this.ctx();
    if (this.isCheckmate(ctx)) return { winner: opposite$1(this.turn) };
...
class Chess extends Position {
  ...
  validate(opts) {
    ...
    return (opts === null || opts === void 0 ? void 0 : opts.ignoreImpossibleCheck)
      ? n.ok(undefined)
  ...
  validEpSquare(square) {
    ...
    if (!this.board.pawn.has(pawn) || !this.board[opposite$1(this.turn)].has(pawn)) return;
    return square;
  }
  castlingDest(side, ctx) {
    ...
    for (const sq of kingPath) {
      if (this.kingAttackers(sq, opposite$1(this.turn), occ).nonEmpty()) return SquareSet.empty();
    }
...
const canPremove = (state, orig, dest) =>
  orig !== dest &&
...
function renderResized$1(state) {
  const asWhite = whitePov(state),
    posToTranslate$1 = posToTranslate(state.dom.bounds());
...
const startDragOrDraw = (s) => (e) => {
  if (s.draggable.current) cancel(s);
    ...
    if (s.drawable.enabled) start$3(s, e);
  } else if (!s.viewOnly) {
...
function getColorMaterial(mesh) {
  ...
  return candidate.color instanceof THREE.Color ? material : null;
}
  ...
  function redrawAll() {
    ...
    const elements = renderWrap(element, maybeState),
      ...
      onResize = () => {
        updateBounds(state);
        renderResized(state);
        if (elements.autoPieces) renderResized$1(state);
      };
